...
function transactionsFor(id, transactionsArray) {
    return transactionsArray.filter(items => items.id === id);
}

function isItemAvailable(id, transactionsArray) {
    let filteredArray = transactionsFor(id, transactionsArray);
    let quantity = filteredArray.reduce((accum, current) => {
