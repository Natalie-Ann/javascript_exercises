...
// identifying each as belonging uniquely to the categories of abundant, perfect, or deficient based on a comparison 
// between the number and the sum of its positive divisors (the numbers that can be evenly divided into the target number with no remainder, 
...
class PerfectNumber {
  static classify(number) {
    ...
    let aliquotSum = findAliquotSum(number);
...
function findAliquotSum(number) {
  let divisors = [];
  ...
  let aliquotSum = divisors.reduce((accum, current) => accum + current, 0);
  return aliquotSum;
}
